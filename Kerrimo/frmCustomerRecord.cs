    ...
    public partial class frmCustomerRecord : Form
    {
        ...
        private void txtCustomers_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM([EMPLOYEE ID])as [Customer ID],RTRIM(Name) as [Customer Name],RTRIM(branch) as [Address],RTRIM(Contact) as [Contact No.], (email) as [Email] from tblUserData where Name  = '" + txtCustomers.Text + "%' and [Priviledge Level] = 2 order by Name", con);
