...
namespace Kerrimo
{
    public partial class frmProductsLog1 : Form
    {
        ...
        public void GetData()
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM(SuppliesID),RTRIM(SuppliesName),RTRIM(Unit),RTRIM(Price),RTRIM(ProductID) from Supplies_List order by SuppliesID", con);
                rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                dataGridView1.Rows.Clear();
                while (rdr.Read() == true)
                {
                    dataGridView1.Rows.Add(rdr[0], rdr[1], rdr[2], rdr[3].ToString(), rdr[4]);
                }
                con.Close();
            }
            catch (Exception ex)
        ...
        private void frmProductsLog1_Load(object sender, EventArgs e)
        {
            GetData();
        }
        ...
        private void frmProductsLog1_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.Hide();
        ...
        private void dataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                frm.txtProductID.Text = dr.Cells[4].Value.ToString();
                frm.label8.Text = label1.Text;
            }
